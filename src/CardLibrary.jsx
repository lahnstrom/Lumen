...
export function CardLibrary({ initialCardId, topic, anki, now, api, refresh, add, edit, review }) {
  ...
  const visible = selectCards(topic.cards, { query, filter, sort, now }).filter(c => !selectedId || c.id === selectedId);
  ...
  return <section className="panel" aria-label="Flashcard library">
    ...
    {!topic.cards.length ? <div className="library-empty"><Layers size={34}/><h3>Turn understanding into recall</h3><p>Ask Lumen for flashcards in your conversation, build a learning kit, or add a card yourself.</p><button className="button secondary" onClick={add}><Plus size={15}/>Add your first card</button></div> : <>
      ...
      {selectedId && <div className="search-card-selection" role="status">Card opened from workspace search<button className="text-button" onClick={() => setSelectedId(null)}>Show all cards <X size={14}/></button></div>}
      <div className="library-tools"><label className="library-search"><Search size={16}/><input aria-label="Search flashcards" placeholder="Search questions, answers, or sources…" value={query} onChange={e => { setQuery(e.target.value); setSelectedId(null); }}/>{query && <button aria-label="Clear flashcard search" onClick={() => setQuery('')}><X size={15}/></button>}</label><label className="library-sort">Sort<select aria-label="Sort flashcards" value={sort} onChange={e => setSort(e.target.value)}><option value="ready">Ready first</option><option value="practised">Most practised</option><option value="alphabetical">Alphabetical</option></select></label></div>
      <div className="library-filters" role="group" aria-label="Filter flashcards">{filters.map(([value, label]) => { const count = selectCards(topic.cards, { filter: value, now }).length; if (value === 'unavailable' && !count && filter !== value) return null; return <button key={value} aria-pressed={filter === value} onClick={() => { setFilter(value); setSelectedId(null); }}>{label}<span>{count}</span></button>; })}</div>
      ...
      {visible.length ? <><div className="card-grid">{visible.slice(0, limit).map(card => <LibraryCard key={card.id} card={card} now={now} edit={edit}/>)}</div>{visible.length > limit && <button className="button secondary library-more" onClick={() => setLimit(n => n + 24)}>Show more cards ({visible.length - limit} remaining)</button>}</> : <div className="library-empty"><Search size={30}/><h3>{query ? 'No matching cards' : 'No cards in this view'}</h3><p>{query ? 'Try a different phrase, source title, or filter.' : 'Your cards move between these views as you study.'}</p><button className="button secondary" onClick={() => { setQuery(''); setFilter('all'); setSelectedId(null); }}>Show all cards</button></div>}
    </>}
  </section>;
}
